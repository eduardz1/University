...
+-comm (succ x) y rewrite +-comm x y 
                        | +-succ y x = refl
...
*-succ zero x = refl
*-succ (succ y) x rewrite +-assoc y x (y * x) 
                        | +-comm y x 
                        | symm (+-assoc x y (y * x))
                        | *-succ y x = refl
  -- begin
...
*-dist-l x y z rewrite *-comm x (y + z) 
                     | *-dist-r y z x 
                     | *-comm y x 
                     | *-comm z x = refl
...
^-assoc x (succ m) n rewrite symm (*-assoc x (x ^ m) (x ^ n)) 
                           | ^-assoc x m n = refl

^-dist : ∀(x y n : ℕ) -> (x * y) ^ n == x ^ n * y ^ n
^-dist x y zero = refl
^-dist x y (succ n) rewrite ^-dist x y n 
                          | *-assoc (x * y) (x ^ n) (y ^ n) 
                          | symm (*-assoc x y (x ^ n)) 
                          | *-comm y (x ^ n) 
                          | *-assoc x (x ^ n) y 
                          | symm (*-assoc (x * x ^ n) y (y ^ n)) = refl
  -- begin
  --   x * y * (x * y) ^ n
...
^-trans x zero n with n 
... | zero = refl
... | succ n₁ rewrite ^-trans x zero n₁ = refl
  -- where
  --   lemma : ∀(n : ℕ) -> 1 ^ n == 1
  --   lemma zero = refl
