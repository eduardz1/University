    ...
    private void bexpr(int label_true, int label_false) {
        switch (look.tag) {
            ...
            case Tag.RELOP -> {
                ...
                switch (relop) {
                    case "<" -> code.emit(OpCode.if_icmplt, label_true); // code.emit(OpCode.if_icmpge, label_false);
                    case ">" -> code.emit(OpCode.if_icmpgt, label_true); // code.emit(OpCode.if_icmple, label_false);
                    case "==" -> code.emit(OpCode.if_icmpeq, label_true); // code.emit(OpCode.if_icmpne, label_false);
                    case "<=" -> code.emit(OpCode.if_icmple, label_true); // code.emit(OpCode.if_icmpgt, label_false);
                    case "<>" -> code.emit(OpCode.if_icmpne, label_true); // code.emit(OpCode.if_icmpeq, label_false);
                    case ">=" -> code.emit(OpCode.if_icmpge, label_true); // code.emit(OpCode.if_icmplt, label_false);
                    default -> error("Error in Word.java RELOP definition");
                }
            }
            case Tag.AND, Tag.OR, Tag.NOT -> bexprp(label_true, label_false);
