...
class GridWorld:
    ...
    def step(self, action):
        if not self.termination:
            ...
            return (
                ...
                reward,
                self.termination,
                [transition_msgs, reward_msgs, termination_msgs],
            )
    ...
    def _compute_transition(self, action):
        ...
        if action == const.LEFT:
            if y == 0:
                return self.current_position, "I can not move out of the world"
            elif self.world[x, y - 1] == const.WALL:
                return self.current_position, "I can not move past the wall"
            else:
                return Loc(x, y - 1), "I moved"

        elif action == const.UP:
            if x == 0:
                return self.current_position, "I can not move out of the world"
            elif self.world[x - 1, y] == const.WALL:
                return self.current_position, "I can not move past the wall"
            else:
                return Loc(x - 1, y), "I moved"
    ...
    def print_unicode(self):
        ...
        for i in range(self.height):
            rows.append(
                [
                    const.translate_to_unicode(int(printworld[i, j]))
